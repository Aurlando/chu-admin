...
function StatutBadge({ statut = "Actif", dark }) {
  const map = {
    ...
    "En activité":dark ? "bg-emerald-400/10 text-emerald-400 border-emerald-500/20" : "bg-emerald-50 text-emerald-600 border-emerald-200",
...
function PersonnelList({ dark, onSelectId, onSelectIdUpdate, onNavigate }) {
  ...
  useEffect(() => {
    fetch(`${API_BASE}/staff/departments`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json()).then(j => setDepartments(j.data || [])).catch(() => {});
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    fetch(`${API_BASE}/staff/fonctions`, { headers: { Authorization: `Bearer ${token}` } })
      .then(r => r.json()).then(j => setFonctions(j.data || [])).catch(() => {});
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const resetFiltres     = () => { setSearch(""); setFilterDept(""); setFilterFonc(""); setPage(1); };
    ...
    <div className="flex-1 p-4 lg:p-6 overflow-auto">
      ...
      <div className={`rounded-2xl border p-4 mb-5 ${T.card}`}>
        <div className="flex flex-wrap gap-3 items-center">
          ...
          {hasActiveFilter && (
            <button onClick={resetFiltres} className={`flex items-center gap-1.5 text-xs px-3 py-2.5 rounded-xl border transition-all ${
