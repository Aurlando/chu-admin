...
function formatStatut(raw = "") {
    ...
    // Capitaliser proprement les mots en fallback
    return normalized
        .split(/\s+/)
...
function PersonnelList({
    ...
    const hasActiveFilter =
        search || filterDept || filterFonc || selectedTypes.length > 0;
        ...
        <div className="flex-1 p-4 lg:p-6 overflow-auto">
            ...
            <div className={`rounded-2xl border p-4 mb-5 ${T.card}`}>
                <div className="flex flex-col gap-4">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:flex lg:flex-row gap-3 items-center">
                        ...
                        <div className="relative w-full lg:w-64 shrink-0">
                            <select
                                value={filterDept}
                                onChange={(e) =>
                                    handleFilterDept(e.target.value)
                                }
                        ...
                        <div className="relative w-full lg:flex-1">
                            <select
                                value={filterFonc}
                                onChange={(e) =>
                                    handleFilterFonc(e.target.value)
                                }
                                className={`w-full text-sm pl-3 pr-7 py-2.5 rounded-xl border outline-none cursor-pointer transition-all ${T.select}`}
                            >
                    ...
                    <div
                        className={`flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pt-4 border-t ${dark ? "border-white/10" : "border-slate-100"}`}
                    >
                        <div className="flex flex-wrap items-center gap-2">
                            <span
                                className={`text-xs font-medium mr-1 ${T.sub}`}
                            >
                                Types :
                            </span>
            ...
            {!loading && error && (
                <div
                    className={`rounded-2xl border p-5 flex items-center gap-3 text-sm ${dark ? "bg-rose-500/10 border-rose-500/20 text-rose-400" : "bg-rose-50 border-rose-200 text-rose-600"}`}
                >
                    <svg
                        xmlns="http://www.w3.org/2000/svg"
            ...
            {!loading && !error && (
                <div className={`rounded-2xl border overflow-x-auto ${T.card}`}>
                    ...
                    {pagination.totalPages > 1 && (
                        <div
                            className={`flex flex-col sm:flex-row items-center justify-between gap-4 px-5 py-3.5 border-t ${dark ? "border-white/8" : "border-slate-100"}`}
                        >
