...
//     → transmise à Sidebar qui affiche le bouton
// ════════════════════════════════════════════════════

const API_URL = "http://localhost:3000/dashboard";

// Données fictives pour les logs d'audit (pas d'API encore)
const auditLogs = [
...
// ── Composant toggle Nuit/Jour (propre, sans bouton Déco dedans)
function ThemeToggle({ dark, onToggle }) {
  return (
    <button
      onClick={onToggle}
      title={dark ? "Passer en mode jour" : "Passer en mode nuit"}
      className={`relative w-[52px] h-7 rounded-full border transition-all duration-300 flex items-center flex-shrink-0 ${
    ...
    >
      ...
      <span className={`absolute w-5 h-5 rounded-full shadow-md transition-all duration-300 flex items-center justify-center text-[10px] ${
        dark ? "translate-x-1 bg-slate-900 text-white" : "translate-x-[26px] bg-white text-yellow-500"
      }`}>
        {dark ? "🌙" : "☀️"}
      </span>
    </button>
...
function StatCard({ icon, label, value, trend, color, dark }) {
  ...
  return (
    <div className={`relative group overflow-hidden rounded-2xl border p-5 transition-all duration-300 hover:-translate-y-0.5 ${
      dark ? "bg-white/5 border-white/10 hover:border-white/20" : "bg-white border-slate-200 hover:border-blue-200 shadow-sm hover:shadow-md"
    }`}>
...
export default function Dashboard({ onLogout }) {
  ...
  // ── Chargement des données API au montage du composant
  useEffect(() => {
    fetch(API_URL)
  ...
  // Tokens thème (classes Tailwind regroupées pour éviter la répétition)
  const T = {
  ...
  return (
    <div className={`min-h-screen font-sans flex transition-colors duration-300 ${T.pageBg}`}>
      ...
      {/* Sidebar — reçoit onLogout pour le bouton de déconnexion */}
      <Sidebar
        activeNav={activeNav}
      ...
      {/* Contenu principal — décalé selon l'état du sidebar */}
      <main className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out ${sidebarCollapsed ? "lg:ml-[68px]" : "lg:ml-64"}`}>
        ...
        <header className={`sticky top-0 z-10 backdrop-blur-md border-b px-4 lg:px-6 py-3 flex items-center gap-4 transition-colors duration-300 ${T.headerBg}`}>
          ...
          {/* Actions topbar : Toggle thème + Notif + Settings */}
          <div className="flex items-center gap-2 ml-auto">

            {/* Toggle Nuit/Jour — propre, sans bouton Déco */}
                  ...
                  <div className="space-y-3">
                    {auditLogs.map((log) => (
                      <div key={log.id} className="flex items-start gap-3">
                        <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 ${dark ? `${log.darkBg} ${log.darkColor}` : `${log.lightBg} ${log.lightColor}`}`}>{log.icon}</div>
