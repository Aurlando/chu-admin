...
function ThemeToggle({ dark, onToggle }) {
    return (
        ...
        >
            <span className="absolute left-1.5 text-[11px] select-none">
                🌙
            </span>
            <span className="absolute right-1.5 text-[11px] select-none">
                ☀️
            </span>
            <span
                className={`absolute w-5 h-5 rounded-full shadow-md transition-all duration-300 flex items-center justify-center text-[10px] ${dark ? "translate-x-1 bg-slate-900 text-white" : "translate-x-6.5 bg-white text-yellow-500"}`}
            >
                {dark ? "🌙" : "☀️"}
            </span>
        </button>
...
function StatCard({ icon, label, value, trend, color, dark }) {
    ...
    return (
        <div
            className={`relative group overflow-hidden rounded-2xl border p-5 transition-all duration-300 hover:-translate-y-0.5 ${dark ? "bg-white/5 border-white/10 hover:border-white/20" : "bg-white border-slate-200 hover:border-blue-200 shadow-sm hover:shadow-md"}`}
        >
            <div
            ...
            <div className="flex items-start justify-between mb-4">
                ...
                <span
                    className={`text-xs font-semibold px-2 py-0.5 rounded-full ${isPositive ? "text-emerald-500 bg-emerald-50" : "text-rose-500 bg-rose-50"}`}
                >
                    {isPositive ? "+" : ""}
                    {trend}%
                </span>
            ...
            <div
                className={`text-3xl font-bold font-mono tracking-tight ${dark ? "text-white" : "text-slate-800"}`}
            >
                {value}
            </div>
            <div
...
function BarChart({ data, dark }) {
    ...
    return (
        <div className="flex items-end gap-1.5 h-40 w-full">
            {data.map((item, i) => {
                const height =
                    max > 0 ? (parseInt(item.id_medecin) / max) * 100 : 5;
                return (
                    <div
                        key={i}
                                ...
                                <div
                                    ...
                                    style={{
                                        background: dark
                                            ...
                                            : "linear-gradient(180deg, #bfdbfe, #3b82f6)",
                                    }}
                                />
                            </div>
                            <div
                                className={`absolute -top-7 left-1/2 -translate-x-1/2 text-[10px] px-1.5 py-0.5 rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap border z-10 ${dark ? "bg-slate-800 text-white border-white/10" : "bg-slate-800 text-white border-slate-700"}`}
                            >
                                {item.id_medecin} médecin
                                {parseInt(item.id_medecin) > 1 ? "s" : ""}
                            </div>
                        </div>
                        <span
                            className={`text-[9px] group-hover:text-blue-500 transition-colors text-center leading-tight font-medium ${dark ? "text-slate-500" : "text-slate-400"}`}
                        >
                            {item.code_service}
                        </span>
                    </div>
                ...
                <>
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 lg:gap-4 mb-6">
                        ...
                        <StatCard
                            ...
                            value={data.summary.total_medecin}
                            trend={-1.2}
                            color="#ef4444"
                        />
                        <StatCard
                            ...
                            label="Sages-femmes"
                            value={data.summary.total_sage_femme}
                            trend={4.8}
                            color="#10b981"
                        />
                        <StatCard
                            ...
                            value={data.summary.total_admin}
                            trend={0.5}
                            color="#8b5cf6"
                        />
                    </div>
                    <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mb-6">
                        <div
                            className={`xl:col-span-2 border rounded-2xl p-5 transition-colors duration-300 ${T.cardBg}`}
                        >
                            <div className="flex items-start justify-between mb-5">
                                <div>
                                    <h2
                                        className={`text-base font-semibold ${T.cardTitle}`}
                                    >
                                ...
                                <select
                                    className={`text-xs border rounded-lg px-2.5 py-1.5 outline-none cursor-pointer transition-colors ${dark ? "bg-white/5 border-white/10 text-slate-400 hover:border-white/20" : "bg-slate-100 border-slate-200 text-slate-500 hover:border-slate-300"}`}
                                >
                        ...
                        <div
                            className={`border rounded-2xl p-5 transition-colors duration-300 ${T.cardBg}`}
                        >
                            <div className="flex items-start justify-between mb-4">
                                <div>
                                    <h2
                            ...
                            <div className="space-y-3">
                                {auditLogs.map((log) => (
                                    ...
                                    >
                                        <div
                                            className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${dark ? `${log.darkBg} ${log.darkColor}` : `${log.lightBg} ${log.lightColor}`}`}
                                        >
                                            {log.icon}
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div
                                                className={`text-[10px] mt-0.5 ${T.cardSub}`}
                                            >
                                                {log.detail}
                                            </div>
                                            <div
                            ...
                            <button
                                className={`mt-4 w-full text-xs border rounded-xl py-2 transition-all ${T.logBtnBorder}`}
                            >
                                Voir tous les logs →
                            </button>
                        </div>
...
export default function Dashboard({ onLogout }) {
    ...
    const T = {
        ...
        logBtnBorder: dark
            ? "border-blue-500/20 hover:border-blue-400/30 text-blue-400 hover:text-blue-300"
    ...
    return (
        ...
        >
            ...
            <main
                className={`flex-1 flex flex-col min-w-0 transition-all duration-300 ease-in-out ${sidebarCollapsed ? "lg:ml-17" : "lg:ml-64"}`}
            >
                {/* Topbar — toujours visible quelle que soit la page active */}
                <header
                    className={`sticky top-0 z-10 backdrop-blur-md border-b px-4 lg:px-6 py-3 flex items-center gap-4 transition-colors duration-300 ${T.headerBg}`}
                >
                    <button
                    ...
                    <div className="flex items-center gap-2 ml-auto">
                        ...
                        />
                        <button
                            className={`relative w-9 h-9 rounded-xl border flex items-center justify-center transition-all ${T.iconBtn}`}
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className="w-4 h-4"
                        ...
                        <button
                            className={`w-9 h-9 rounded-xl border flex items-center justify-center transition-all ${T.iconBtn}`}
                        >
                            <svg
                                xmlns="http://www.w3.org/2000/svg"
                                className="w-4 h-4"
