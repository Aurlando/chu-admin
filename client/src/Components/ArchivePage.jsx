...
// ────────────────────────────────────────
function DeleteModal({ open, onClose, onConfirm, staff, deleting, deleteError, dark }) {
    useEffect(() => {
            ...
            ${dark ? "bg-rose-500/15" : "bg-rose-50"}`}>
                    ...
                    <div className="text-center space-y-1.5">
                        ...
                        <p className={`text-sm leading-relaxed ${sub}`}>
                            ...
                            de la base de données. Cette action est <strong className="text-rose-500">irréversible</strong>.
                        </p>
                    </div>
...
export default function ArchivePage({ dark }) {
    ...
    const handleDelete = () => {
        ...
        fetch(`${API_BASE}/staff/${modal.staff.id}`, {
            ...
            .then(() => {
                ...
                showToast(`${modal.staff.nom} ${modal.staff.prenoms} supprimé définitivement.`);
                // Recharge la liste
                fetchArchives();
            })
    ...
    const tdCls    = dark ? "px-5 py-[13px] border-b border-white/4" : "px-5 py-[13px] border-b border-slate-100";
    ...
    return (
        <div className={`flex-1 overflow-auto ${bg}`}>
            ...
            <DeleteModal
                open={modal.open}
                onClose={() => { if (!deleting) { setModal({ open: false, staff: null }); setDeleteError(null); } }}
        ...
        ${dark ? "bg-[#0d1526] border border-white/10 text-slate-200" : "bg-slate-800 text-white"}`}>
                <svg className={`w-4 h-4 flex-shrink-0 ${toast.type === "success" ? "text-emerald-400" : "text-rose-400"}`}
                ...
                <div className="flex items-start justify-between gap-4 flex-wrap">
                    <div>
                        ...
                        <p className={`text-sm mt-1 max-w-lg leading-relaxed ${sub}`}>
                            Personnel ayant terminé leur service. Vous pouvez les supprimer définitivement de la base de données.
                        </p>
                    </div>
                    {/* Compteur */}
                    {!loading && !error && (
          ...
          ${dark ? "bg-rose-500/8 border-rose-500/15 text-rose-400"
                 : "bg-rose-50 border-rose-200 text-rose-800"}`}>
                    ...
                    <span>
                        <strong>Suppression définitive</strong> — La suppression retire le personnel de toutes les tables.
                  ...
                  transition-all font-[inherit] ${inputCls}`}
                    ...
                    {error && (
                        <div className="px-5 pt-4">
                            <ErrorAlert message={`Impossible de charger : ${error}`} onRetry={fetchArchives} dark={dark} />
                        </div>
                    )}
                    ...
                    {!loading && !error && (
                        <div className="overflow-x-auto">
                            <table className="w-full border-collapse text-sm">
                                <thead>
                                    <tr>
                                        <th className={`${thCls} text-left`}>Personnel</th>
                                        <th className={`${thCls} text-left`}>Service</th>
                                        <th className={`${thCls} text-left`}>Fonction</th>
                                        <th className={`${thCls} text-left`}>Date de sortie</th>
                              ...
                              text-[11px] font-bold text-white flex-shrink-0 opacity-70"
                                                        style={{ background: avatarColor(p.id) }}
                                                    >
                                                        {initials(p.nom, p.prenoms)}
                                                    </div>
                                                    <div>
                                                        <p className={`font-semibold text-[13px] ${ttl}`}>
                                                            {p.nom} {p.prenoms}
                                                        </p>
                                            ...
                                            {/* Service */}
                                            <td className={tdCls}>
                                                <span className={`text-[12.5px] ${sub}`}>
                                                    {p.service ?? "—"}
                                                </span>
                                            </td>
                                            {/* Fonction */}
                            ...
                            ${dark
                                                    ...
                                                    {p.date_sortie
                                                        ? new Date(p.date_sortie).toLocaleDateString("fr-FR")
                                                        : "—"}
                                                </span>
                                            </td>
                                            {/* Bouton supprimer */}
                              ...
                              ${dark
                                                            ? "border-rose-500/25 text-rose-400 hover:bg-rose-500/10 hover:border-rose-500/40"
                                        ...
                                        <tr>
                                            <td colSpan={5}>
                                                <div className={`flex flex-col items-center py-16 gap-3 ${sub}`}>
                                                    ...
                                                    <p className="text-sm font-medium">Aucun personnel archivé.</p>
                                                </div>
                                            </td>
                                        </tr>
                                    )}
                    ...
                    {!loading && !error && total > PER_PAGE && (
                        <div className={`flex items-center justify-between px-5 py-3.5 border-t ${border}`}>
                            <p className={`text-xs font-medium ${sub}`}>
                                Affichage de {startIdx}–{endIdx} sur {total} archivés
                            </p>
