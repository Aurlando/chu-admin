...
export default function NotificationMenu({ dark, onNavigate, refreshKey }) {
    ...
    const totalNotifs = notifStats
        ? notifStats.depasse + notifStats.tres_proche
        : 0;
            ...
            {showNotifMenu && (
                <div
                    className={`absolute right-0 mt-2 w-80 rounded-2xl border shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200 ${
                        dark
                            ? "bg-[#0d1526] border-white/10 shadow-black/40"
                            : "bg-white border-slate-200 shadow-slate-200"
                    }`}
                >
                    <div
                        className={`px-4 py-3 border-b flex items-center justify-between ${dark ? "border-white/5 bg-white/3" : "border-slate-100 bg-slate-50"}`}
                    >
                        <h3
                            className={`text-xs font-bold uppercase tracking-wider ${dark ? "text-slate-400" : "text-slate-500"}`}
                        >
                            Notifications
                        </h3>
                        {totalNotifs > 0 && (
                    ...
                    <div className="max-h-96 overflow-y-auto">
                        {totalNotifs === 0 ? (
                            <div className="p-8 text-center">
                                <div
                                    className={`w-12 h-12 rounded-full mx-auto mb-3 flex items-center justify-center ${dark ? "bg-white/5 text-slate-600" : "bg-slate-100 text-slate-400"}`}
                                >
                                    <svg
                                        xmlns="http://www.w3.org/2000/svg"
                                        className="w-6 h-6"
                                ...
                                <p
                                    className={`text-sm font-semibold ${dark ? "text-slate-300" : "text-slate-700"}`}
                                >
                                    Aucune notification
                                </p>
                                <p
                                    className={`text-xs mt-1 ${dark ? "text-slate-500" : "text-slate-500"}`}
                                >
                            ...
                            <div className="divide-y dark:divide-white/5 divide-slate-100">
                                {notifStats.depasse > 0 && (
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p
                                                className={`text-[13px] font-bold ${dark ? "text-white" : "text-slate-800"}`}
                                            >
                                                Avancements dépassés
                                            </p>
                                            <p
                                            ...
                                            >
                                                ...
                                                personnel
                                                {notifStats.depasse > 1
                                                    ? "s ont"
                                                    : " a"}{" "}
                                                dépassé la date d'avancement.
                                            </p>
                                        </div>
                                    </button>
                                )}
                    ...
                    <button
                        ...
                        className={`w-full py-3 text-center text-xs font-bold transition-all border-t ${
                            dark
                                ? "bg-white/3 border-white/5 text-blue-400 hover:bg-white/8 hover:text-blue-300"
