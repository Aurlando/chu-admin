                  ...
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                      className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${
                        dark ? "border-white/10 text-slate-300 hover:bg-white/8" : "border-slate-200 text-slate-600 hover:bg-slate-100 cursor-pointer"
                      }`}>
                      {photoPreview ? "Changer" : "Choisir"}
                    </button>
                    {photoPreview && (
                      <button type="button" onClick={() => { setPhotoFile(null); setPhotoPreview(null); }}
                        className="text-xs text-rose-400 hover:text-rose-300 transition-colors cursor-pointer">Supprimer</button>
                    )}
                  </div>
                </div>
              </div>
              ...
              {creerCompte && (
                <div className={`rounded-xl border p-4 space-y-4 ${dark ? "bg-white/3 border-white/8" : "bg-slate-50 border-slate-200"}`}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <Field label="Mot de passe" required dark={dark} error={errors.password}> {/* [MODIFIÉ] */}
                      <div className="relative"> {/* [AJOUTÉ] Conteneur pour l'icône */}
                        <input
                          type={showPassword ? "text" : "password"} // [MODIFIÉ] Type dynamique
                        ...
                        <button
                          type="button"
                          onClick={() => setShowPassword(!showPassword)} // [AJOUTÉ] Toggle la visibilité
            ...
            {currentStep < STEPS.length ? (
              ...
              disabled={submitting}
                ...
                {submitting ? (
                  ...
                  <>
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 cursor-pointer" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                    </svg>
                    Enregistrer
                  </>
