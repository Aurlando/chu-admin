        ...
        <div className="flex-1 flex items-center justify-center p-6">
          <div className="text-center max-w-sm">
            <div className="w-16 h-16 rounded-full bg-rose-100 flex items-center justify-center mx-auto mb-4">
              <svg xmlns="http://www.w3.org/2000/svg" className="w-8 h-8 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const STEPS = [
  {
    id: 1, label: "Identité",
    icon: (active) => (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
  ...
  },
  {
    ...
    icon: (active) => (
      <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={active ? 2.5 : 2}>
        <path d="M12 14l9-5-9-5-9 5 9 5z"/><path strokeLinecap="round" strokeLinejoin="round" d="M12 14l6.16-3.422a12.083 12.083 0 01.665 6.479A11.952 11.952 0 0112 20.055a11.952 11.952 0 00-6.824-2.998 12.078 12.078 0 01.665-6.479L12 14zm-4 6v-7.5l4-2.222"/>
      </svg>
    ),
  },
  {
...
function Field({ label, required, children, error, dark }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className={`text-[11px] font-bold uppercase tracking-widest ${dark ? "text-slate-500" : "text-slate-400"}`}>
...
function RecapRow({ label, value, dark }) {
  return (
    <div className={`flex items-start justify-between py-2 border-b last:border-0 ${dark ? "border-white/5" : "border-slate-100"}`}>
      <span className={`text-xs ${dark ? "text-slate-500" : "text-slate-400"}`}>{label}</span>
      <span className={`text-xs font-semibold text-right max-w-[60%] ${dark ? "text-slate-200" : "text-slate-700"}`}>
...
export function AddPersonnelInner({ dark, onAnnuler }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/staff/departments`, { headers })
      ...
      .then(json => {
        ...
        setServices(typeof data[0] === "string" ? data.map((s, i) => ({ id: i + 1, libelle: s })) : data);
      }).catch(() => {});
    fetch(`${API_BASE}/staff/fonctions`, { headers })
      .then(r => r.json())
      .then(json => {
        const data = json.data || [];
        setFonctions(typeof data[0] === "string" ? data.map((f, i) => ({ id: i + 1, libelle: f })) : data);
      }).catch(() => {});
  ...
  const handleDiplomeChange = (index, field, value) =>
    setDiplomes(prev => prev.map((d, i) => i === index ? { ...d, [field]: value } : d));

  const ajouterDiplome  = () => setDiplomes(prev => [...prev, { ...DIPLOME_VIDE }]);
  const supprimerDiplome = (i) => setDiplomes(prev => prev.filter((_, idx) => idx !== i));
  ...
  const progressPercent = STEPS.length <= 1 ? 100 : Math.round(((currentStep - 1) / (STEPS.length - 1)) * 100);
  ...
  const dipCard = dark ? "bg-white/3 border-white/8 rounded-xl border p-4 space-y-3"
                       : "bg-slate-50 border-slate-200 rounded-xl border p-4 space-y-3";

  if (success) {
    return (
      <div className={`flex-1 flex items-center justify-center p-6 ${bg}`}>
        <div className="text-center max-w-sm">
          ...
          <h2 className={`text-xl font-bold mb-2 ${ttl}`}>Personnel enregistré !</h2>
          <p className={`text-sm mb-6 ${sub}`}>Le nouveau membre du personnel a été ajouté avec succès dans le système.</p>
          <button onClick={() => setSuccess(false)}
            className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold rounded-xl transition-all shadow-lg shadow-blue-600/20">
  ...
  return (
    <div className={`flex-1 overflow-auto ${bg}`}>
      <div className="max-w-2xl mx-auto p-4 lg:p-6">
        ...
        {/* ── En-tête ── */}
        <div className="mb-6">
          <h1 className={`text-2xl lg:text-3xl font-bold ${ttl}`}>Ajouter un Personnel</h1>
          <p className={`text-sm mt-1 ${sub}`}>Complétez les informations en {STEPS.length} étapes</p>
        </div>
        ...
        <div className={`rounded-2xl border p-5 mb-5 ${card}`}>
            ...
            {STEPS.map((step, idx) => {
              ...
              return (
                <div key={step.id} className="flex items-center flex-1 last:flex-none">
                  ...
                  <div className="flex flex-col items-center gap-1.5 shrink-0">
                    <button
                      ...
                      className={`w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 border-2 ${
                          ...
                          : active
                          ? dark ? "border-blue-500 bg-blue-500/15 text-blue-400 cursor-default" : "border-blue-500 bg-blue-50 text-blue-600 cursor-default"
                          : dark ? "border-white/10 bg-white/3 text-slate-600 cursor-default"    : "border-slate-200 bg-slate-50 text-slate-400 cursor-default"
                      }`}>
                      {done ? (
                    ...
                    <span className={`hidden sm:block text-[10px] font-semibold whitespace-nowrap transition-colors ${
                      active ? dark ? "text-blue-400" : "text-blue-600"
                             : done ? dark ? "text-slate-300" : "text-slate-600"
                  ...
                  {!last && (
                    <div className={`flex-1 h-0.5 mx-2 rounded-full overflow-hidden ${dark ? "bg-white/8" : "bg-slate-200"}`}>
                      <div
          ...
          <div className="mt-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className={`text-[11px] font-medium ${sub}`}>Progression globale</span>
              <span className={`text-[11px] font-bold ${dark ? "text-blue-400" : "text-blue-600"}`}>
                {progressPercent}%
              </span>
            </div>
            <div className={`w-full h-1.5 rounded-full ${dark ? "bg-white/8" : "bg-slate-200"}`}>
              <div
        ...
        {apiError && (
          <div className="mb-4 rounded-2xl border border-rose-500/20 bg-rose-500/10 p-3.5 flex items-center gap-3 text-rose-400 text-sm">
            ...
            {apiError}
            <button onClick={() => setApiError(null)} className="ml-auto opacity-60 hover:opacity-100">✕</button>
          </div>
        )}
        ...
        <div className={`rounded-2xl border ${card}`}>
          ...
          {currentStep === 1 && (
            <div className="p-5 lg:p-6 space-y-5">
              <div className={`flex items-center gap-2 pb-4 border-b ${dark ? "border-white/8" : "border-slate-100"}`}>
                ...
                <div>
                  <h2 className={`text-sm font-bold ${ttl}`}>Identité du membre</h2>
                  <p className={`text-xs ${sub}`}>Photo, nom complet et informations de base</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-5">
                <div
                  ...
                  className={`relative w-20 h-20 rounded-2xl overflow-hidden shrink-0 cursor-pointer group border-2 border-dashed transition-all ${
                    photoPreview
                      ? "border-transparent"
                      : dark ? "border-white/15 hover:border-blue-500/50" : "border-slate-300 hover:border-blue-400"
                  }`}
                >
                  {photoPreview ? (
                    <img src={photoPreview} alt="Aperçu" className="w-full h-full object-cover" />
                  ) : (
                    <div className={`w-full h-full flex flex-col items-center justify-center gap-1 ${dark ? "bg-white/5" : "bg-slate-50"}`}>
                      <svg xmlns="http://www.w3.org/2000/svg" className={`w-6 h-6 ${dark ? "text-slate-600" : "text-slate-400"}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
                      </svg>
                      <span className={`text-[9px] font-medium ${dark ? "text-slate-600" : "text-slate-400"}`}>Photo</span>
                    </div>
                  )}
                  {/* Overlay hover */}
                  {photoPreview && (
                ...
                <div>
                  <p className={`text-sm font-medium ${ttl}`}>Photo de profil</p>
                  <p className={`text-xs mt-0.5 mb-2 ${sub}`}>JPG, PNG ou WEBP — max 5 Mo</p>
                  <div className="flex items-center gap-2">
                    <button type="button" onClick={() => fileInputRef.current?.click()}
                      className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-all ${
                        dark ? "border-white/10 text-slate-300 hover:bg-white/8" : "border-slate-200 text-slate-600 hover:bg-slate-100"
                      }`}>
                      {photoPreview ? "Changer" : "Choisir"}
                    </button>
                    {photoPreview && (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Nom" required dark={dark} error={errors.nom}>
                  <input type="text" placeholder="Ex: RAKOTO" value={form.nom}
                    onChange={e => handleChange("nom", e.target.value)} className={inp("nom")} />
                </Field>
                <Field label="Prénoms" required dark={dark} error={errors.prenoms}>
                  <input type="text" placeholder="Ex: Jean Paul" value={form.prenoms}
                    onChange={e => handleChange("prenoms", e.target.value)} className={inp("prenoms")} />
                </Field>
          ...
          {currentStep === 2 && (
            <div className="p-5 lg:p-6 space-y-5">
              <div className={`flex items-center gap-2 pb-4 border-b ${dark ? "border-white/8" : "border-slate-100"}`}>
                ...
                <div>
                  <h2 className={`text-sm font-bold ${ttl}`}>Situation administrative</h2>
                  <p className={`text-xs ${sub}`}>Catégorie, classe, échelon et statut</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Catégorie" required dark={dark} error={errors.categorie}>
                ...
                <Field label="Classe" required dark={dark} error={errors.classe}>
                ...
                <Field label="Échelon" required dark={dark} error={errors.echelon}>
                  <select value={form.echelon} onChange={e => handleChange("echelon", e.target.value)}
          ...
          {currentStep === 3 && (
            <div className="p-5 lg:p-6 space-y-5">
              <div className={`flex items-center gap-2 pb-4 border-b ${dark ? "border-white/8" : "border-slate-100"}`}>
                ...
                <div>
                  <h2 className={`text-sm font-bold ${ttl}`}>Affectation & Contact</h2>
                  <p className={`text-xs ${sub}`}>Service, fonction et coordonnées</p>
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <Field label="Service" required dark={dark} error={errors.service_id}>
                  <select value={form.service_id} onChange={e => handleChange("service_id", e.target.value)}
                    className={sel("service_id")}>
                    <option value="">Sélectionner un service</option>
                    {services.map(s => <option key={s.id} value={s.id}>{s.libelle}</option>)}
                  </select>
                </Field>
                <Field label="Fonction" required dark={dark} error={errors.fonction_id}>
                  <select value={form.fonction_id} onChange={e => handleChange("fonction_id", e.target.value)}
          ...
          {currentStep === 4 && (
            <div className="p-5 lg:p-6 space-y-5">
              <div className={`flex items-center gap-2 pb-4 border-b ${dark ? "border-white/8" : "border-slate-100"}`}>
                ...
                <div>
                  <h2 className={`text-sm font-bold ${ttl}`}>Diplômes & Formation</h2>
                  <p className={`text-xs ${sub}`}>Renseignez le diplôme principal et les diplômes supplémentaires</p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border ${
                        diplome.est_principal
                          ? dark ? "text-blue-400 border-blue-500/20 bg-blue-500/10" : "text-blue-600 border-blue-200 bg-blue-50"
                          : dark ? "text-slate-500 border-white/10 bg-white/5"        : "text-slate-500 border-slate-200 bg-slate-100"
                      }`}>
                        {diplome.est_principal ? "★ Principal" : `Diplôme ${index + 1}`}
                      </span>
                      {!diplome.est_principal && (
                        <button type="button" onClick={() => supprimerDiplome(index)}
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <Field label="Année d'obtention" dark={dark}>
                        <input type="number" min="1950" max={new Date().getFullYear()}
                          placeholder="Ex: 2010"
                          value={diplome.annee_obtention}
                          onChange={e => handleDiplomeChange(index, "annee_obtention", e.target.value)}
                          className={inputCls} />
                      </Field>
                    </div>
                  </div>
                ...
                <button type="button" onClick={ajouterDiplome}
                  className={`w-full py-2.5 rounded-xl border text-sm font-medium flex items-center justify-center gap-2 transition-all ${
                    dark ? "border-white/10 border-dashed text-slate-500 hover:text-blue-400 hover:border-blue-500/30 hover:bg-blue-500/5"
          ...
          {currentStep === 5 && (
            <div className="p-5 lg:p-6 space-y-5">
              <div className={`flex items-center gap-2 pb-4 border-b ${dark ? "border-white/8" : "border-slate-100"}`}>
                ...
                <div>
                  <h2 className={`text-sm font-bold ${ttl}`}>Compte & Récapitulatif</h2>
                  <p className={`text-xs ${sub}`}>Vérifiez les informations avant de valider</p>
                </div>
              </div>

              {/* Case à cocher compte */}
              <label className={`flex items-start gap-3 cursor-pointer select-none p-4 rounded-xl border transition-all ${
                creerCompte
                  ? dark ? "border-blue-500/30 bg-blue-500/8" : "border-blue-300 bg-blue-50"
                  : dark ? "border-white/8 bg-white/3 hover:border-white/15" : "border-slate-200 bg-slate-50 hover:border-slate-300"
              }`}>
                <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 mt-0.5 transition-all ${
                  ...
                  <input type="checkbox" className="sr-only" checked={creerCompte}
                    onChange={e => {
                      setCreerCompte(e.target.checked);
                      if (!e.target.checked) { handleChange("username",""); handleChange("password",""); }
                    }} />
                </div>
                <div>
                  <p className={`text-sm font-semibold ${ttl}`}>Créer un compte d'accès HIS</p>
              ...
              {creerCompte && (
                <div className={`rounded-xl border p-4 space-y-4 ${dark ? "bg-white/3 border-white/8" : "bg-slate-50 border-slate-200"}`}>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <Field label="Nom d'utilisateur" required dark={dark} error={errors.username}>
                      <input type="text" placeholder="Ex: jean.rakoto" value={form.username}
                        onChange={e => handleChange("username", e.target.value)}
                        className={inp("username")} autoComplete="new-password" />
                    </Field>
                    <Field label="Mot de passe" required dark={dark} error={errors.password}>
                      <input type="password" placeholder="Minimum 6 caractères" value={form.password}
              ...
              {/* Récapitulatif */}
              <div className={`rounded-xl border overflow-hidden ${dark ? "border-white/8" : "border-slate-200"}`}>
                <div className={`px-4 py-2.5 text-[11px] font-bold uppercase tracking-widest ${dark ? "bg-white/5 text-slate-500 border-b border-white/8" : "bg-slate-50 text-slate-400 border-b border-slate-200"}`}>
                  Récapitulatif
                </div>
                <div className="px-4 py-1">
                  ...
                  <RecapRow dark={dark} label="Service" value={services.find(s => String(s.id) === String(form.service_id))?.libelle} />
                  <RecapRow dark={dark} label="Fonction" value={fonctions.find(f => String(f.id) === String(form.fonction_id))?.libelle} />
                  <RecapRow dark={dark} label="Téléphone" value={form.telephone} />
                  <RecapRow dark={dark} label="Email" value={form.email} />
                  {/* Tous les diplômes — on boucle sur le tableau diplomes */}
                  {diplomes.map((d, i) => (
                    <RecapRow
                      key={i}
                      dark={dark}
                      label={d.est_principal ? "Diplôme principal" : `Diplôme ${i + 1}`}
                      value={d.libelle}
                    />
                  ))}
                  {/* Compte : on affiche uniquement si créer un compte est coché */}
                  {creerCompte && (
                    <RecapRow dark={dark} label="Compte HIS" value={form.username ? `@${form.username}` : "Non renseigné"} />
                  )}
                </div>
              </div>
            </div>
          ...
          <div className={`flex items-center justify-between gap-3 px-5 lg:px-6 py-4 border-t ${dark ? "border-white/8 bg-white/2" : "border-slate-100 bg-slate-50/50"} rounded-b-2xl`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button type="button" onClick={goPrev} disabled={currentStep === 1}
                className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm font-medium transition-all disabled:opacity-30 disabled:cursor-not-allowed ${
                  dark ? "border-white/10 text-slate-400 hover:text-white hover:bg-white/5"
            ...
            <span className={`text-xs ${sub}`}>{currentStep} / {STEPS.length}</span>

            {/* Droite : Suivant ou Enregistrer */}
            {currentStep < STEPS.length ? (
              <button type="button" onClick={goNext}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-500 text-white text-sm font-semibold transition-all shadow-lg shadow-blue-600/20 hover:-translate-y-0.5">
                Suivant
                <svg xmlns="http://www.w3.org/2000/svg" className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
