...
const GROUP_PALETTE = [
    {
        dot: "bg-violet-500",
        dark: "bg-violet-500/12 text-violet-400 border-violet-500/20",
        light: "bg-violet-50 text-violet-700 border-violet-200",
    },
    {
        dot: "bg-blue-500",
        dark: "bg-blue-500/12 text-blue-400 border-blue-500/20",
        light: "bg-blue-50 text-blue-700 border-blue-200",
    },
    {
        dot: "bg-emerald-500",
        dark: "bg-emerald-500/12 text-emerald-400 border-emerald-500/20",
        light: "bg-emerald-50 text-emerald-700 border-emerald-200",
    },
    {
        dot: "bg-rose-500",
        dark: "bg-rose-500/12 text-rose-400 border-rose-500/20",
        light: "bg-rose-50 text-rose-700 border-rose-200",
    },
    {
        dot: "bg-amber-500",
        dark: "bg-amber-500/12 text-amber-400 border-amber-500/20",
        light: "bg-amber-50 text-amber-700 border-amber-200",
    },
    {
        dot: "bg-cyan-500",
        dark: "bg-cyan-500/12 text-cyan-400 border-cyan-500/20",
...
function CellBtn({ value, onClick, dark, groupIdx }) {
    if (!value || value === 0) {
        return (
            <span
                className={`text-sm ${dark ? "text-white/15" : "text-slate-300"}`}
            >
                —
            </span>
        );
    ...
    return (
        ...
        >
            ...
            >
                {/* Barre colorée */}
                <div
                    className={`absolute top-0 left-0 right-0 h-0.5 ${pal.dot}`}
                />

                {/* ── En-tête ── */}
                <div
                    className={`flex items-start justify-between px-6 pt-6 pb-4 border-b ${hdBorder}`}
                >
              ...
              ${dark ? "bg-white/6" : "bg-slate-50 border border-slate-200"}`}
                        >
                            <svg
                                className={`w-5 h-5 ${dark ? "text-slate-300" : "text-slate-500"}`}
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
                                strokeWidth={1.7}
                        ...
                        <div className="min-w-0">
                            <h3
                                className={`text-[13.5px] font-bold leading-snug truncate ${ttl}`}
                            >
                                {serviceName}
                            </h3>
                  ...
                  ${dark ? pal.dark : pal.light}`}
                                >
                                    <span
                                        className={`w-1.5 h-1.5 rounded-full ${pal.dot}`}
                                    />
                                    {groupeName}
                                </span>
                                {data && (
                                    <span
                                        className={`text-[11px] font-medium ${sub}`}
                                    >
                                        {data.total} agent
                                        {data.total > 1 ? "s" : ""}
                                    </span>
              ...
              transition-all cursor-pointer ${closeBtn}`}
                ...
                {/* ── Corps ── */}
                <div
                    className="px-6 py-5 max-h-[62vh] overflow-y-auto space-y-4
                ...
                >
                    ...
                    {data && !loading && (
                        <>
                            ...
                            {data.comptage_par_fonction &&
                                Object.keys(data.comptage_par_fonction).length >
                                    0 && (
                                    <div>
                                        <p
                                            className={`text-[10px] font-bold uppercase tracking-widest mb-2.5 ${sub}`}
                                        >
                                            Répartition par fonction
                                        </p>
                          ...
                          text-[10px] font-bold text-white bg--to-br ${avatarBg(i)}`}
                            ...
                            <div
                                className={`h-px ${dark ? "bg-white/5" : "bg-slate-100"}`}
                            />
            ...
            <div className="max-w-6xl mx-auto px-5 py-6 space-y-6">
              ...
              ${
                ...
                <div className={`rounded-2xl border overflow-hidden ${card}`}>
                    {/* Header */}
                    <div
                        className={`flex items-center justify-between px-5 py-4 border-b ${dark ? "border-white/6" : "border-slate-100"}`}
                    >
                        <div>
                            <p className={`text-[13px] font-bold ${ttl}`}>
                                Tableau croisé — Personnel par Service &amp;
                                Groupe
                            </p>
                            <p className={`text-xs mt-0.5 ${sub}`}>
                                Cliquez sur un chiffre pour afficher la liste
                                nominative
                            </p>
                        </div>
                        {!loading && !error && recap && (
                            <span
                                className={`hidden sm:inline-flex items-center gap-1 px-3 py-1 rounded-full border text-[11px] font-semibold
                ${
                    dark
                        ...
                        : "bg-blue-50 border-blue-200 text-blue-600"
                            ...
                            >
                                {lignes.length} services · {groupes.length}{" "}
                                groupes
                            </span>
                        )}
                    </div>
                    ...
                    {!loading && !error && recap && lignes.length > 0 && (
                        <div
                            className={`flex items-center gap-2.5 px-5 py-2.5 border-b text-xs font-medium
              ${
                  dark
                      ...
                      : "bg-blue-50/70 border-blue-100 text-blue-600"
                        ...
                        >
                            ...
                            Les chiffres colorés sont cliquables — ils affichent
                            la liste nominative du personnel de ce groupe.
                        </div>
                    )}
