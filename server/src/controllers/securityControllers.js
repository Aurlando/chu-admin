...
async function getAccounts(req, res) {
    ...
    try {
        const result = await securityModels.getAccounts({ search, page, limit: safeLimit });
        res.status(200).json({
...
async function resetPassword(req, res) {
    ...
    if (typeof new_password !== 'string' || new_password.trim().length < 6) {
        return res.status(400).json({ message: "Le mot de passe doit être une chaîne d'au moins 6 caractères." });
    }
...
async function toggleActif(req, res) {
    ...
    try {
        ...
        res.status(200).json({
            message: result.actif ? 'Compte activé avec succès' : 'Compte désactivé avec succès',
            actif:   result.actif,
        });
...
async function getAuditLog(req, res) {
    ...
    try {
        const result = await securityModels.getAuditLog({ page, limit: safeLimit });
        res.status(200).json({
            data: result.data,
            pagination: result.pagination,
        });
